...
def generate_stub(project_id: str, evm_path: Path, mc_path: Path):
    ...
    return {
        ...
        "id": project_id,
        "kpis": {
            "CPI": float(last.get("CPI", 1)),
            "SPI": float(last.get("SPI", 1)),
            "EAC": float(last.get("EAC", 0)),
            "VAC": float(last.get("VAC", 0)),
            "P80_EAC": float(summ.get("EAC_P80", 0)),
            "P80_FinishDate": None,
        },
        ...
        "contributors": [
            {
                "name": "Top drivers",
                "type": "risk",
                "impact_dollars": float(summ.get("EAC_P80", 0) - summ.get("EAC_P50", 0)),
                "impact_days": float(summ.get("Finish_P80", 0) - summ.get("Finish_P50", 0)),
            }
        ],
    }
